...
const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    // console.log("datat in actions",userInfo);
    // console.log("datat in action1",order);
...
const payOrder = (order, paymentResult) => async (dispatch, getState) => {
  console.log("paymennnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnt",)
  try {
    dispatch({ type: ORDER_PAY_REQUEST, payload: paymentResult });
    const { userSignin: { userInfo } } = getState();
    const body={order,paymentResult}
    const { data } = await axios.post("http://localhost:5000/api/orders/pay", body, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
